...
export default function ExpertLayout({
  ...
  const [userName, setUserName] = useState("专家用户")
  const [isLoggingOut, setIsLoggingOut] = useState(false)

  useEffect(() => {
    const user = authStorage.getUser()
    if (user) {
      const displayName = user.realName || user.expertName || user.username || "专家用户"
      setUserName(displayName)
      console.log("[v0] Expert layout loaded user:", displayName)
    }
  }, [])
