...
export async function POST(request: NextRequest) {
  try {
    ...
    if (project.status !== PROJECT_STATUS.EXECUTING) {
      return NextResponse.json({ code: 400, message: "只有执行中的项目可以提交年度检查", data: null }, { status: 400 })
    }
    ...
    annualChecks.push(newCheck)

    // 更新项目状态
    const index = projects.findIndex((p) => p.id === projectId)
    projects[index] = {
      ...project,
      status: PROJECT_STATUS.ANNUAL_CHECKING as any,
      statusName: "年度检查中",
    ...
    console.error("[v0] 年度检查提交失败:", error)
