...
export default function AnnualReviewPage() {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      await annualApi.review(selectedCheck.id, {
        result: reviewResult,
      ...
      toast({
        ...
        description: `年度检查已${reviewResult === "pass" ? "提交至监管端" : "驳回"}`,
      })
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">年度检查审核</h1>
          <p className="text-muted-foreground mt-2">审核项目负责人提交的年度检查材料</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>查看和审核项目年度检查</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {loading ? (
                  ...
                  checkList.map((check) => (
                    <TableRow key={check.id}>
                      ...
                      <TableCell>{((check.budgetAnnual || 0) / 10000).toFixed(0)}万元</TableCell>
                      <TableCell>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <Dialog>
                            ...
                            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                              <DialogHeader>
                                <DialogTitle>年度检查详情</DialogTitle>
                                <DialogDescription>
                                ...
                                <Tabs defaultValue="content" className="w-full">
                                  ...
                                  <TabsContent value="content" className="space-y-4">
                                    <div className="grid grid-cols-4 gap-4">
                                      ...
                                      <div>
                                        <Label className="text-muted-foreground">年度预算</Label>
                                        <div className="mt-1 flex items-center gap-1">
                                          <DollarSign className="h-4 w-4 text-muted-foreground" />
                                          <span className="font-medium">
                                            {((selectedCheck.budgetAnnual || 0) / 10000).toFixed(2)}万元
                                          </span>
                                        </div>
                                      </div>
                                      <div>
                                    ...
                                    {selectedCheck.status !== "pending" && (
                                      <>
                                        <div>
                                          <Label className="text-muted-foreground">工作总结</Label>
                                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                                            {selectedCheck.workSummary || "暂无"}
                                          </p>
                                        </div>
                                        <div>
                                          <Label className="text-muted-foreground">已完成任务</Label>
                                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                                            {selectedCheck.completedTasks || "暂无"}
                                          </p>
                                        </div>
                                        <div>
                                          <Label className="text-muted-foreground">取得成果</Label>
                                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                                            {selectedCheck.achievements || "暂无"}
                                          </p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                          <Label className="text-muted-foreground">改进措施</Label>
                                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                                            {selectedCheck.improvements || "暂无"}
                                          </p>
                                        </div>
                                        <div>
                                          <Label className="text-muted-foreground">下一年度计划</Label>
                                          <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                                            {selectedCheck.nextYearPlan || "暂无"}
                                          </p>
                                        </div>
                                        ...
                                        <div className="flex gap-4">
                                          ...
                                          <div className="flex-1 pb-4">
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                              {selectedCheck.submittedBy} 提交了年度检查材料
                                            </p>
                                          </div>
                                        </div>
                                      )}
      ...
      <Dialog open={reviewDialogOpen} onOpenChange={setReviewDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>年度检查审核</DialogTitle>
            <DialogDescription>请对年度检查进行审核并填写审核意见</DialogDescription>
          </DialogHeader>
