...
const MidtermCheckPage = () => {
  const router = useRouter()
  const { toast } = useToast()
  const [midterms, setMidterms] = useState<any[]>([])
  ...
  const loadMidterms = async () => {
    try {
      setLoading(true)
      console.log("[v0] Loading midterms with params:", {
        current: currentPage,
        size: pageSize,
        projectNumber,
        projectName,
      ...
      console.log("[v0] Midterm API response:", response)
      if (response.data) {
        const listData = response.data.records || response.data.list || []
        const totalCount = response.data.total || listData.length
        console.log("[v0] Midterms loaded:", listData.length, "items, total:", totalCount)
        setMidterms(listData)
        setTotal(totalCount)
      }
    } catch (error) {
      console.error("[v0] Failed to load midterms:", error)
      toast({
        title: "加载失败",
    ...
    if (checkStatus === "draft" || checkStatus === "pending") {
      return (
        <div className="flex gap-2">
          <Button variant="link" size="sm" onClick={() => router.push(`/dashboard/projects/midterm/${item.id}`)}>
            详情
          </Button>
          <Button variant="link" size="sm" onClick={() => router.push(`/dashboard/projects/midterm/${item.id}/edit`)}>
            编辑
          </Button>
        </div>
      )
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(midterms.map((item) => item.id))
    } else {
      setSelectedIds([])
    }
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">项目中期检查</h1>
        <p className="text-sm text-muted-foreground mt-1">管理和提交项目中期检查报告</p>
      </div>
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-12">
                  <Checkbox checked={selectedIds.length === midterms.length} onCheckedChange={handleSelectAll} />
                </TableHead>
                <TableHead>项目编号</TableHead>
                <TableHead>项目名称</TableHead>
                <TableHead>检查状态</TableHead>
                <TableHead>中期检查</TableHead>
                <TableHead>中期检查时间</TableHead>
              ...
              {midterms.length === 0 ? (
                ...
                midterms.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.checkYear || item.midtermCheck}</TableCell>
...
export default MidtermCheckPage
